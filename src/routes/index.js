...
router.get('/genres', async (req, res)=> {
    const apiURL2 = await axios.get(`https://api.rawg.io/api/genres?key=${RAWG_API_KEY}`)
...
router.get('/platforms', async (req, res)=> {
    const apiURL3 = await axios.get(`https://api.rawg.io/api/platforms?key=${RAWG_API_KEY}`)
...
router.get('/videogame/:id', async (req, res)=> {
    const {id} = req.params
    if(!id.includes('-')) {
        const detail = await axios.get(`https://api.rawg.io/api/games/${id}?key=${RAWG_API_KEY}`)
